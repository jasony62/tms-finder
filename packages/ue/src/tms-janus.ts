...
export const JANUS_SERVER = (() => {
  ...
  if (window.location.protocol === 'http:') {
    server = `http://${address}`
    if (process.env.VUE_APP_TMS_JANUS_HTTP_PORT && parseInt(process.env.VUE_APP_TMS_JANUS_HTTP_PORT) > 0)
      server += `:${parseInt(process.env.VUE_APP_TMS_JANUS_HTTP_PORT)}`
  } else {
    server = `https://${address}`
    if (process.env.VUE_APP_TMS_JANUS_HTTPS_PORT && parseInt(process.env.VUE_APP_TMS_JANUS_HTTPS_PORT) > 0)
